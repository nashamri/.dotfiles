;;; nas-screenshot.el --- screenshots integrated with org attachment dirs
...
;; the screenshot command. The link to the file will be placed at
...
(defcustom nas-screenshot-command-line "import %f"
  "contains the command line used to take a screenshot. You need
   ...
   by %f" :group 'nas-screenshot)

(defcustom nas-screenshot-relative-links t
...
concatenation of the attachment dir and the filename"
  :type 'boolean :group 'nas-screenshot)

;;;###autoload
(defun nas-screenshot (prfx filename)
...
`nas-screenshot-get-attach-dir' function.
...
customized using the `nas-screenshot-command-line' variable.
...
the links being already placed inside the text."
  ...
  (if (equal major-mode 'org-mode)
      (let ((scrfilename (concat (file-name-as-directory
          (nas-screenshot-get-attach-dir))
				 filename))
	    linkfilename status)
  (if nas-screenshot-relative-links
	    (setq linkfilename
		  (file-relative-name
	...
	(if (and (file-exists-p scrfilename)
		 (not (y-or-n-p (format "%s already exists. Overwrite?"
					scrfilename))))
      (call-interactively 'nas-screenshot)
    ...
    (let* ((arglst (split-string nas-screenshot-command-line " "))
		 (cmd (car arglst))
...
(defun nas-screenshot-get-attach-dir ()
...
(provide 'nas-screenshot)

;;; nas-screenshot.el ends here
